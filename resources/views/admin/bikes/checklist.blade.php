...
<div class="container mt-5">
    <!-- Month Filter -->
    <form method="GET" action="{{ route('checkbike.index') }}" class="mb-4">
            ...
            <tbody>
                @foreach ($bikes as $bike)
                            <tr>
                                ...
                                @for ($date = \Carbon\Carbon::parse($month . '-01'); $date->lte(\Carbon\Carbon::parse($month . '-01')->endOfMonth()); $date->addDay())
                                                @php
                                                    $isBooked = $bike->bookings->contains(function ($booking) use ($date) {
                                                        return $date->between(
                                                    ...
                                                    $isServiceDate = $bike->services->contains(function ($service) use ($date) {
                                                        return $date->between(
