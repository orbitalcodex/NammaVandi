...
<nav class="navbar navbar-expand-lg"
    ...
    <div class="container">
        ...
        <button class="navbar-toggler" type="button" data-toggle="collapse" data-target="#ftco-nav"
            ...
            <span class="fas fa-bars" style="color: #E87C1E; font-size: 20px;"></span>
        </button>
...
<div class="container mt-4 ">
    <div class="row justify-content-between align-items-center mb-5">
        ...
        <div class="col-md-7 text-md-left text-center heading-section ftco-animate">
            <h2 class="font-weight-bold" style="color: rgb(57, 37, 14);">BIKE &nbsp; AVAILABILITY</h2>
        </div>

        <!-- Button Section on Right -->
        <div class="col-6 col-md-2 text-center mb-2">
            <a href="{{ route('getbikechecklist') }}"
...
<div class="container mb-4 mt-2 p-2">
    <div class="card shadow-sm border-0 rounded-3"
        ...
        <div class="card-body">
            <form method="GET" action="{{ route('getbikebooked') }}" id="dateFilterForm"
...
<div class="container">
    <!-- Booked Bikes Section -->
    <div class="card shadow-sm border-0 rounded-3 mb-4">
        <div class="card-header"
            style="background-color:   rgb(57, 37, 14); color: #FFF; border-bottom: 3px solid rgb(110, 25, 14);">
            <h3 class="mb-0" style="color: white;"><i class="bi bi-motorcycle"></i> BOOKED BIKES</h3>
        </div>
        <div class="card-body">
            @if($bookedBikes->count() > 0)
                @foreach($bookedBikes as $bike)
                    <div class="row mb-4 car-card">
                        <div class="col-md-4">
                            <img src="data:image/jpeg;base64,{{ $bike->bike_image }}" alt="{{ $bike->bike_brandname }}"
                                class="img-fluid rounded-3 shadow hover-zoom">
                        </div>
                        <div class="col-md-8">
                            <h4 style="color:  rgb(57, 37, 14);">
                                <i class="bi bi-motorcycle"></i>
                                {{ $bike->bike_companyname }} - {{ $bike->bike_brandname }}
                            </h4>
                            <h5 class="text-muted">{{ $bike->bike_model }}</h5>
                            <div class="mt-3">
            ...
            @else
                <div class="alert alert-info"
                    ...
                    <i class="bi bi-info-circle"></i> No booked bikes in this range.
                </div>
            @endif
        </div>
    </div>
    ...
    <div class="card shadow-sm border-0 rounded-3">
        <div class="card-header"
            style="background-color:  rgb(57, 37, 14); color: #FFF; border-bottom: 3px solid rgb(110, 25, 14);">
            <h3 class="mb-0" style="color: white;"><i class="bi bi-motorcycle"></i> FREE BIKES</h3>
        </div>
        <div class="card-body">
            @if($freeBikes->count() > 0)
                <div class="row g-4">
                    @foreach($freeBikes as $bike)
                        <div class="col-md-4">
                            <div class="card h-100 shadow-sm hover-lift" style="border: 1px solid  rgb(57, 37, 14);">
                                <img src="data:image/jpeg;base64,{{ $bike->bike_image }}" class="card-img-top"
                                    alt="{{ $bike->bike_brandname }}">
                                <div class="card-body">
                                    <h5 class="card-title" style="color:  rgb(57, 37, 14);">
                                        <i class="bi bi-motorcycle"></i>
                                        {{ $bike->bike_companyname }} - {{ $bike->bike_brandname }}
                                    </h5>
                                    <p class="card-text">{{ $bike->bike_model }}</p>
                                </div>
                            </div>
                        </div>
                    @endforeach
            ...
            @else
                <div class="alert alert-info"
                    style="background-color: #FFF4E6; color:  rgb(57, 37, 14); border-left: 5px solid rgb(110, 25, 14);">
                    <i class="bi bi-info-circle"></i> No free bikes in this range.
                </div>
            @endif
        </div>
    </div>
